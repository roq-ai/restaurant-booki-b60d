...
import { useRoqClient, useChefFindManyWithCount } from 'lib/roq';
...
export function ChefListPage(props: ChefListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'restaurant', 'menu.count'],
  ...
  const { data, error, isLoading, mutate } = useChefFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.chef.delete({ where: { id } });
      await mutate();
    } catch (error) {
