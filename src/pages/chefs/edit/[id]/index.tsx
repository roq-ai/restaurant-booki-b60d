...
import { useRoqClient, useChefFindFirst } from 'lib/roq';
...
function ChefEditPage() {
  ...
  const { data, error, isLoading, mutate } = useChefFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ChefInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.chef.update({
        data: values as RoqTypes.chef,
        where: {
          id,
        },
      });
